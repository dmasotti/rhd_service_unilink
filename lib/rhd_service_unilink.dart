...
class UniLinkService extends RhdCore.RhdSettingsService {
  ...
  Future<void> init({RhdCore.Settings? set}) async {
    ...
    _sub_uni_links = AppLinks().uriLinkStream.listen((Uri? uri) {
      ...
      set.doLog('UniLink error (' + err.toString() + ') ',
          type: RhdCore.AlertType.error, loglevel: RhdCore.LogLevel.error);
    });
  }
  ...
  @override
  Future<void> onAfterServiceInit(
      {RhdCore.Settings? set, RhdCore.RhdSettingsService? service}) async {}

  /// cancella apertura automatica dell 'istanza RHD
  @override
  RhdCore.Item? filterAutoItemOpen(
      {RhdCore.Settings? set, RhdCore.Item? item}) {
    return item;
  }
  ...
  Map<String, Map<String, dynamic>>? getMessages({RhdCore.Settings? set}) {
    return {
      ...
      "it": {
        "os_user_id":
            RhdUtil.MessageLookupByLibrary.simpleMessage("OS user id"),
  ...
  Map<String, Map<String, dynamic>>? getSettings(
      {String? itemAppId, RhdCore.Settings? set}) {
    if (itemAppId == null ||
        set == null ||
  ...
  Future<void> _evalQueryParameters(Map addDt, RhdCore.Settings _set) async {
    ...
    if (addDt.containsKey("b64")) {
      ...
      _item.navigateUrl = jsp["navigate_url"];
      _item.url = jsp["url"];
      _item.instance = jsp["instance"];
      _item.userId = jsp["user_id"];

      _item.userFirstName = jsp['userFirstName'] ?? '';
      _item.userLastName = jsp['userLastName'] ?? '';
      _item.userLocale = jsp['userLocale'] ?? 'it';

      _item.pcolor = jsp['pcolor'] ?? '' as Color?;
      _item.scolor = jsp['scolor'] ?? '' as Color?;
      _item.totpssec = jsp['totpssec'] ?? '';
      _item.image = jsp['image'] ?? '';
      _item.cutomername = jsp['customername'] ?? '';

      _item.token = jsp["jSessionID"];
      _item.appId = jsp["appId"];

      _item.notificationsUrl = jsp["notifications_url"] ?? '';
      _item.altUdid = jsp["alt_udid"];
      _item.accessToken = jsp["access_token"];
      ...
      Timer(const Duration(milliseconds: 50), () {
        _set.pushSignalStateForward(_item,
            appId: _item.appId,
            icon: null,
            body: null,
            title: null,
    ...
    if (addDt.containsKey("d")) {
      ...
      try {
        ...
        _set.doLog(barcode,
            type: RhdCore.AlertType.info, loglevel: RhdCore.LogLevel.debug);
        _titem = RhdCore.Item(-1, jsp["appId"], jsp["name"], jsp["user_id"],
            jsp["refresh_token"], jsp["tokenurl"]);
        ...
        if (_titem != null) {
          ...
          Icon? icc;
          Color cp = _titem.pcolor ??
              Theme.of(RhdUtil.Utils.homeAppKey.currentContext!)
                  .colorScheme
                  .secondary;
          ...
          RhdUtil.Utils.showNotifyBar(
            ...
            buttonAction: () async {
              String? tok = await _set.getBarcodeToken(barcodeCl, _titem,
                  disableCertificate: false); //this.disableCertificate
              ...
              if (tok == null || tok == "") {
                ...
                toverwrite =
                    RhdCore.ItemsRepository().checkIstanceExist(_titem);
                //bool res = true;
                if (toverwrite) {
                  mmsg = RhdUtil.RHDLocalizations.of(
                      RhdUtil.Utils.homeAppKey.currentContext!,
                ...
                if (_titem.disableSSLCertificate) {
                  ...
                  mmsg += "\n" +
                      RhdUtil.RHDLocalizations.of(
                          RhdUtil.Utils.homeAppKey.currentContext!,
                          'home_sslcert_confirm'); //"errore cert";;
                }
              }
              // faccio vedere
              ...
              bool res = await (RhdUtil.Utils.showModalDialog(
                ...
                content: RhdUtil.RHDLocalizations.of(
                    RhdUtil.Utils.homeAppKey.currentContext!, 'text_rhd_add'),
                actions: <Widget>[
                  CupertinoButton(
                      onPressed: () async {
                        _titem.status = 1;
                        ...
                        if (nitm != null) {
                          ir.save();
                          Navigator.of(RhdUtil.Utils.homeAppKey.currentContext!)
                              .pop(false); //chiude la modale
                          //TODO lancia RHD
                          Timer(const Duration(milliseconds: 50), () {
                            _set.pushSignalStateForward(nitm,
                                appId: appId,
                                tabId: tabId,
                                module: module,
                                command: command,
                      ...
                      child: Text(RhdUtil.RHDLocalizations.of(
                          RhdUtil.Utils.homeAppKey.currentContext!,
                          'butt_confirm'))),
                  CupertinoButton(
                    onPressed: () {
    ...
    if (itm != null) {
      ...
      if (xbody == '') {
        xbody = RhdUtil.RHDLocalizations.of(
            RhdUtil.Utils.homeAppKey.currentContext!, 'linknot_body_received');
      }
      Icon? icc;
      Color cp = itm?.pcolor ??
          Theme.of(RhdUtil.Utils.homeAppKey.currentContext!)
      ...
      RhdUtil.Utils.showNotifyBar(
        ...
        buttonAction: () async {
          ...
          Timer(const Duration(milliseconds: 50), () {
            _set.pushSignalStateForward(itm,
                appId: appId,
                tabId: tabId,
                module: module,
                command: command,
